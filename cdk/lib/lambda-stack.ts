...
export class LambdaStack extends cdk.Stack {
  constructor(
    ...
    const lambdaRole = iam.Role.fromRoleArn(
      this,
      `${envs.APP_NAME}-lambda-role-${envs.ENV}`,
      envs.LAMBDA_ROLE_ARN,
      {
        mutable: false,
      },
    );
    ...
    const webBucket = s3.Bucket.fromBucketArn(
      this,
      envs.WEB_BUCKET,
      webBucketArn,
    );
    ...
    const serverLambda = new lambda.Function(
      this,
      `${envs.APP_NAME}-server-${envs.ENV}`,
      {
    ...
    new cloudfront.Distribution(
      this,
      `${envs.APP_NAME}-cloudfront-${envs.ENV}`,
      {
        ...
        defaultBehavior: {
          ...
          viewerProtocolPolicy:
            cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS, // 强制 HTTPS
        },
        // 添加针对静态文件的行为，指向 S3
        additionalBehaviors: {
